...
package body Cbe.Sync_Superblock with
   Spark_Mode
is
   ...
   is
      ...
      --  Pending_Item
      --
      function Invalid_Item return Item_Type is (Sta => Invalid, Idx => 0, Gen => 0);
      ...
      procedure Set_State (Obj : in out Item_Type;
                           Sta :        State_Type)
      is
      begin
         Obj.Sta := Sta;
   ...
   function Initialized_Object return Object_Type is
      (Current_Item => Item.Invalid_Item, Current_Primitive => Primitive.Invalid_Object);
   ...
   is
   begin
      if Item.Complete (Obj.Current_Item) then
   ...
   function Peek_Completed_Generation (Obj  : Object_Type;
                                       Prim : Primitive.Object_Type) return Generation_Type
   is
   begin
      if Item.Complete (Obj.Current_Item)
         and Primitive.Block_Number (Obj.Current_Primitive) = Primitive.Block_Number (Prim)
      then
   ...
   is
   begin
      if Item.Pending (Obj.Current_Item) then
         return Obj.Current_Primitive;
      end if;
      return Primitive.Invalid_Object;
   ...
   --
   function Peek_Generated_Index (Obj  : Object_Type;
                                  Prim : Primitive.Object_Type) return Superblock_Index_Type
   is
   begin
   ...
   is
   begin
      if Item.In_Progress (Obj.Current_Item)
         and Primitive.Block_Number (Obj.Current_Primitive) = Primitive.Block_Number (Prim)
      then
