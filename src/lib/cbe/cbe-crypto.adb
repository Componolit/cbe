...
is
   ...
   is
      ...
      --  Execute
      --
      procedure Execute (Obj : in out Item_Type;
                         Key :        Key_Type)
      is
      begin
         case Primitive.Operation (Obj.Prim) is
            when Write =>
               Aes_Cbc_4k.Encrypt
                  (Key, Aes_Cbc_4k.Block_Number_Type (Primitive.Block_Number (Obj.Prim)), Obj.Plain_Data,
                   Obj.Cipher_Data);
      ...
      --
      procedure Mark_Completed_Primitive (Obj : in out Item_Type;
                                          Prm :        Primitive.Object_Type;
                                          Key :        Key_Type)
      is
      begin
         if Obj.State /= In_Progress or not Primitive.Equal (Obj.Prim, Prm) then
            return;
         end if;
         if Primitive.Operation (Obj.Prim) = Read and Primitive.Success (Prm) then
            Aes_Cbc_4k.Decrypt
               (Key, Aes_Cbc_4k.Block_Number_Type (Primitive.Block_Number (Obj.Prim)), Obj.Cipher_Data, Obj.Plain_Data);
         end if;
      ...
      --
      function Item_Complete (Item : Item_Type;
                              Prim : Primitive.Object_Type) return Boolean is
         (Item.State = Complete and Primitive.Equal (Item.Prim, Prim));

      --
      --  Copy_Decrypted_Data
      --
      procedure Copy_Decrypted_Data (Item       :     Item_Type;
      ...
      begin
         ...
         if Primitive.Operation (Item.Prim) = Read and Primitive.Success (Prim) then
      ...
      --  Copy_Encrypted_Data
      --
      procedure Copy_Encrypted_Data (Item        :     Item_Type;
                                     Prim        :     Primitive.Object_Type;
      ...
      begin
         ...
         if Primitive.Operation (Item.Prim) = Write and Primitive.Success (Prim) then
      ...
      --  Invalid_Object
      --
      function Invalid_Object return Item_Type is
         (State       => Invalid, Prim => Primitive.Invalid_Object, Plain_Data => (others => 0),
      ...
      function Submitted_Encryption_Object (Prm       : Primitive.Object_Type;
                                            Plain_Dat : Plain_Data_Type) return Item_Type is
         (State => Submitted, Prim => Prm, Plain_Data => Plain_Dat, Cipher_Data => (others => 0));

      --
      --  Submitted_Decryption_Object
      --
      function Submitted_Decryption_Object (Prm        : Primitive.Object_Type;
                                            Cipher_Dat : Cipher_Data_Type) return Item_Type is
         (State => Submitted, Prim => Prm, Plain_Data => (others => 0), Cipher_Data => Cipher_Dat);
      ...
      procedure State (Obj : in out Item_Type;
                       Sta :        State_Type)
      is
      begin
         Obj.State := Sta;
   ...
   --  Initialized_Object
   --
   function Initialized_Object (Key : Key_Type) return Object_Type is
      (Key => Key, Items => (others => Item.Invalid_Object), Execute_Progress => False);

   --
   --  Primitive_Acceptable
   --
   function Primitive_Acceptable (Obj : Object_Type) return Boolean is
      (for some Itm of Obj.Items => Item.Invalid (Itm));

   --
   --  Submit_Encryption_Primitive
   --
   procedure Submit_Encryption_Primitive (Obj        : in out Object_Type;
                                          Prim       :        Primitive.Object_Type;
   ...
   begin
      ...
      for Item_Id in Obj.Items'Range loop
         if Item.Invalid (Obj.Items (Item_Id)) then
            Obj.Items (Item_Id) := Item.Submitted_Encryption_Object (Prim_Buf, Plain_Data);
            exit Items_Loop;
         end if;
      end loop Items_Loop;
   ...
   --
   procedure Submit_Decryption_Primitive (Obj         : in out Object_Type;
                                          Prim        :        Primitive.Object_Type;
   ...
   begin
      ...
      for Item_Id in Obj.Items'Range loop
         if Item.Invalid (Obj.Items (Item_Id)) then
            Obj.Items (Item_Id) := Item.Submitted_Decryption_Object (Prim_Buf, Cipher_Data);
            exit Items_Loop;
         end if;
      end loop Items_Loop;
   ...
   --  Execute
   --
   procedure Execute (Obj : in out Object_Type) is
   begin
   ...
   procedure Copy_Decrypted_Data (Obj        :     Crypto.Object_Type;
                                  Prim       :     Primitive.Object_Type;
   ...
   procedure Copy_Encrypted_Data (Obj         :     Crypto.Object_Type;
                                  Prim        :     Primitive.Object_Type;
                                  Cipher_Data : out Cipher_Data_Type)
   is
   begin
