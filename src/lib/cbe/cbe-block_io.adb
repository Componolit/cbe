...
package body Cbe.Block_Io with
   Spark_Mode
is

   function Invalid_Entry return Entry_Type is
      (Orig_Tag => Tag_Invalid, Prim => Primitive.Invalid_Object, State => Unused);
   ...
   --  Initialized_Object
   --
   function Initialized_Object return Object_Type is (Entries => (others => Invalid_Entry), Used_Entries => 0);
   function Primitive_Acceptable (Obj : Object_Type) return Boolean is (Obj.Used_Entries < Num_Entries_Type'Last);
   ...
   function Peek_Completed_Data_Index (Obj : Object_Type) return Data_Index_Type
   is
   begin
      for I in Obj.Entries'Range loop
         if Obj.Entries (I).State = Complete then
   ...
   function Peek_Completed_Tag (Obj  : Object_Type;
                                Prim : Primitive.Object_Type) return Cbe.Tag_Type
   is
   begin
      for I in Obj.Entries'Range loop
         if Obj.Entries (I).State = Complete and Primitive.Equal (Prim, Obj.Entries (I).Prim) then
   ...
   function Peek_Generated_Data_Index (Obj  : Object_Type;
                                       Prim : Cbe.Primitive.Object_Type) return Data_Index_Type
   is
   begin
      for I in Obj.Entries'Range loop
         --  XXX why is the condition different from
         --     'Peek_Generated_Primitive' and 'Drop_Completed_Primitive'?
         if Obj.Entries (I).State = Pending or Obj.Entries (I).State = In_Progress then
   ...
   begin
      for I in Obj.Entries'Range loop
         if Obj.Entries (I).State = Pending and Primitive.Equal (Prim, Obj.Entries (I).Prim) then
   ...
   begin
      for I in Obj.Entries'Range loop
         if Obj.Entries (I).State = In_Progress and Primitive.Equal (Prim, Obj.Entries (I).Prim) then
