...
package body Cbe.Cache_Flusher with
   Spark_Mode
is
   ...
   is
      ...
      --  Invalid_Item
      --
      function Invalid_Item return Item_Type is
         (Sta => Invalid, Pba => 0, Idx => Cache.Cache_Index_Type'Last, Suc => False);

      --
      --  Pending_Item
      --
      procedure Pending_Item (Obj : out Item_Type;
      ...
      function Pba (Obj : Item_Type) return Physical_Block_Address_Type is (Obj.Pba);
      function Index (Obj : Item_Type) return Cache.Cache_Index_Type is (Obj.Idx);
      ...
      procedure Set_Success (Obj : in out Item_Type;
                             Suc :        Boolean)
      is
      begin
         Obj.Suc := Suc;
   ...
   --  Initialized_Object
   --
   function Initialized_Object return Object_Type is (Items => (others => Item.Invalid_Item));
   ...
   begin
      ...
      for Item_Id in Obj.Items'Range loop
         if Item.Complete (Obj.Items (Item_Id))
            and Item.Pba (Obj.Items (Item_Id)) = Physical_Block_Address_Type (Primitive.Block_Number (Prim))
         then
   ...
   begin
      Complete_Primitive :
      for Item_Id in Obj.Items'Range loop
         if Item.In_Progress (Obj.Items (Item_Id))
            and Item.Pba (Obj.Items (Item_Id)) = Physical_Block_Address_Type (Primitive.Block_Number (Prim))
         then
            Item.State (Obj.Items (Item_Id), Item.Complete);
            Item.Set_Success (Obj.Items (Item_Id), Boolean (Primitive.Success (Prim)));
            return;
         end if;
