...
package body Cbe.Cache with
   Spark_Mode
is

   package body Cache_Item with
      Spark_Mode
   is
      ...
      --  Unused_Object
      --
      function Unused_Object return Cache_Item_Type is (Pba => 0, Ts => 0, State => Unused, Dirty => False);

      --
      --  Initialize_Object
      --
      procedure Initialize_Object (Obj : out Cache_Item_Type;
      ...
      procedure State (Obj : in out Cache_Item_Type;
                       Sta :        State_Type)
      is
      begin
         Obj.State := Sta;
      ...
      procedure Set_Ts (Obj : in out Cache_Item_Type;
                        Ts  :        Timestamp_Type)
      is
      begin
         Obj.Ts := Ts;
      ...
      procedure Invalidate (Obj : in out Cache_Item_Type) is
      begin
         Obj.State := Unused;
      end Invalidate;

      procedure Mark_Dirty (Obj : in out Cache_Item_Type) is
      begin
         Obj.Dirty := True;
      end Mark_Dirty;

      procedure Mark_Clean (Obj : in out Cache_Item_Type) is
      begin
         Obj.Dirty := False;
      end Mark_Clean;
   ...
   is
      ...
      function Pending_Object (Pba : Physical_Block_Address_Type) return Job_Item_Type is
         (State => Pending, Success => False, Pba => Pba);

      --
      --  Unused_Object
      --
      function Unused_Object return Job_Item_Type is (Pba => 0, State => Unused, Success => False);

      --
      --  Already_Pending
      --
      function Already_Pending (Obj : Job_Item_Type;
                                Pba : Physical_Block_Address_Type) return Boolean
      is
      begin
      ...
      procedure State (Obj : in out Job_Item_Type;
                       Sta :        State_Type)
      is
      begin
         Obj.State := Sta;
      ...
      procedure Set_Unused (Obj : in out Job_Item_Type) is
      begin
         Obj.State := Unused;
      end Set_Unused;
   ...
   function Initialized_Object return Object_Type is
      (Cache_Items => (others => Cache_Item.Unused_Object), Job_Items => (others => Job_Item.Unused_Object),
       Active_Jobs => 0, Execute_Progress => False);

   --
   --  Data_Available
   --
   function Data_Available (Obj : Object_Type;
                            Pba : Physical_Block_Address_Type) return Boolean
   is
   begin
      Item_Loop :
      for Item of Obj.Cache_Items loop
         if Cache_Item.Used (Item) and Cache_Item.Pba (Item) = Pba then
            return True;
         end if;
      end loop Item_Loop;
      return False;
   ...
   --  Data_Index
   --
   function Data_Index (Obj : in out Object_Type;
                        Pba :        Physical_Block_Address_Type;
                        Ts  :        Timestamp_Type) return Cache_Index_Type
   is
   begin
      for Cache_Item_Id in Obj.Cache_Items'Range loop
         if
            Cache_Item.Used (Obj.Cache_Items (Cache_Item_Id))
            and Cache_Item.Pba (Obj.Cache_Items (Cache_Item_Id)) = Pba
         then
   ...
   --  Invalidate
   --
   procedure Invalidate (Obj : in out Object_Type;
                         Pba :        Physical_Block_Address_Type)
   is
   begin
      for Cache_Item_Id in Obj.Cache_Items'Range loop
   ...
   --  Dirty
   --
   function Dirty (Obj : Object_Type;
                   Idx : Cache_Index_Type) return Boolean
   is
   begin
      ---  XXX remove loop and access directly?
      for Cache_Item_Id in Obj.Cache_Items'Range loop
         if Cache_Item_Id = Idx then
   ...
   --  Flush
   --
   function Flush (Obj : Object_Type;
                   Idx : Cache_Index_Type) return Physical_Block_Address_Type
   is
   begin
   ...
   --  Mark_Clean
   --
   procedure Mark_Clean (Obj : in out Object_Type;
                         Pba :        Physical_Block_Address_Type)
   is
   begin
      for Cache_Item_Id in Obj.Cache_Items'Range loop
   ...
   --  Mark_Dirty
   --
   procedure Mark_Dirty (Obj : in out Object_Type;
                         Pba :        Physical_Block_Address_Type)
   is
   begin
      for Cache_Item_Id in Obj.Cache_Items'Range loop
   ...
   --
   function Request_Acceptable_Logged (Obj : Object_Type;
                                       Pba : Physical_Block_Address_Type) return Boolean
   is
   ...
   --
   procedure Submit_Request_Logged (Obj : in out Object_Type;
                                    Pba :        Physical_Block_Address_Type)
   is
   begin
   ...
   begin
      ...
      New_Job :
      for Job_Item_Id in Obj.Job_Items'Range loop
   ...
   --  Fill_Cache
   --
   procedure Fill_Cache (Obj      : in out Object_Type;
   ...
   begin
      ...
      Complete_Job :
      for Job_Item_Id in Obj.Job_Items'Range loop
         if Job_Item.Complete (Obj.Job_Items (Job_Item_Id)) and Job_Item.Success (Obj.Job_Items (Job_Item_Id)) then
   ...
   function Peek_Generated_Data_Index (Obj  : Object_Type;
                                       Prim : Primitive.Object_Type) return Cache_Index_Type
   is
   begin
      Pending_Data :
      for Job_Item_Id in Obj.Job_Items'Range loop
         if Job_Item.Pending (Obj.Job_Items (Job_Item_Id))
            and Job_Item.Pba (Obj.Job_Items (Job_Item_Id)) = Physical_Block_Address_Type (Primitive.Block_Number (Prim))
         then
   ...
   --
   procedure Mark_Completed_Primitive (Obj  : in out Object_Type;
                                       Prim :        Primitive.Object_Type)
   is
   begin
      Complete_Primitive :
      for Job_Item_Id in Obj.Job_Items'Range loop
         if Job_Item.In_Progress (Obj.Job_Items (Job_Item_Id))
            and Job_Item.Pba (Obj.Job_Items (Job_Item_Id)) = Physical_Block_Address_Type (Primitive.Block_Number (Prim))
         then
