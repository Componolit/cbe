...
package body Cbe.Free_Tree with
   Spark_Mode
is
   ...
   is
   begin
      Obj := Initialized_Object (Rt_Pba, Rt_Gen, Rt_Hash, Hght, Degr, Lfs);
   end Initialize_Object;

   function Initialized_Object (Rt_Pba  : Physical_Block_Address_Type;
                                Rt_Gen  : Generation_Type;
                                Rt_Hash : Hash_Type;
                                Hght    : Tree_Level_Type;
                                Degr    : Tree_Degree_Type;
                                Lfs     : Tree_Number_Of_Leafs_Type) return Object_Type
   is
      Tr_Helper : constant Tree_Helper.Object_Type := Tree_Helper.Initialized_Object (Degr, Hght, Lfs);
   begin
      return (Trans_Helper   => Tr_Helper, Trans => Translation.Initialized_Object (Tr_Helper, True),
          Execute_Progress   => False, Do_Update => False, Do_Wb => False, Wb_Done => False, Nr_Of_Blocks => 0,
          Nr_Of_Found_Blocks => 0, Query_Branches => (others => Query_Branch_Invalid), Curr_Query_Branch => 0,
          Found_Pbas         => (others => Pba_Invalid), Free_Pbas => (others => Pba_Invalid), Root_Pba => Rt_Pba,
          Root_Hash          => Rt_Hash, Root_Gen => Rt_Gen, Curr_Query_Prim => Primitive.Invalid_Object,
          Curr_Type_2 => Io_Entry_Invalid, Wb_Ios => (others => Io_Entry_Invalid), Wb_Data => Write_Back_Data_Invalid);
   ...
   function Request_Acceptable (Obj : Object_Type) return Boolean is (Obj.Nr_Of_Blocks = 0);
   ...
   function Leaf_Usable (Active_Snaps     : Snapshots_Type;
                         Last_Secured_Gen : Generation_Type;
                         Node             : Type_Ii_Node_Type) return Boolean
   is
      Free   : Boolean := False;
            ...
            for Snap of Active_Snaps loop
               if Snapshot_Valid (Snap) then
                  ...
                  declare
                     ...
                     Is_Free : constant Boolean         := (F_Gen <= B_Gen or A_Gen >= (B_Gen + 1));
                  begin
   ...
   procedure Execute_Translation (Obj        : in out Object_Type;
   ...
   begin
      ...
      loop
         ...
         --  Print_Primitive (Obj.Curr_Query_Prim);
         --  Print_Line_Break;
         Translation.Submit_Primitive (Obj.Trans, Obj.Root_Pba, Obj.Root_Gen, Obj.Root_Hash, Obj.Curr_Query_Prim);
         --
      ...
      loop
         ...
         declare
            Prim : constant Primitive.Object_Type := Translation.Peek_Generated_Primitive (Obj.Trans);
         begin
            ...
            begin
               if not Cache.Data_Available (Cach, Pba) then
                  ...
                  declare
                     Data_Index : constant Cache.Cache_Index_Type := Cache.Data_Index (Cach, Pba, Timestamp);
                  begin
                     Translation.Mark_Generated_Primitive_Complete (Obj.Trans, Cach_Data (Data_Index), Trans_Data);
      ...
      loop
         ...
         declare
            Prim : constant Primitive.Object_Type := Translation.Peek_Completed_Primitive (Obj.Trans);
         begin
            ...
            Obj.Curr_Type_2 :=
               ...
               --
            if not Translation.Can_Get_Type_1_Info_Spark (Obj.Trans, Prim) then
               --  Print_String ("FRTR Execute could not get type 1 info");
               --  Print_Line_Break;
               raise Program_Error;
            end if;
            Translation.Get_Type_1_Info (Obj.Trans, Obj.Query_Branches (Obj.Curr_Query_Branch).Trans_Infos);
   ...
   procedure Execute_Query (Obj              : in out Object_Type;
   ...
   begin
      ...
      Declare_Nodes_1 :
      declare
         Nodes : Type_Ii_Node_Block_Type with
            Address => Query_Data (0)'Address;
      begin
         For_Type_2_Nodes :
         for Node_Index in Nodes'Range loop
            ...
            Declare_Pba_1 :
            declare
               Pba : constant Physical_Block_Address_Type := Nodes (Node_Index).Pba;
            begin
               if Pba /= 0 then
                  Declare_Usable :
                  declare
                     Usable : constant Boolean := Leaf_Usable (Active_Snaps, Last_Secured_Gen, Nodes (Node_Index));
                  begin
                     if Usable then
                        ...
                        if Obj.Query_Branches (Obj.Curr_Query_Branch).Vba = Vba_Invalid then
                           Obj.Query_Branches (Obj.Curr_Query_Branch).Vba :=
                              Virtual_Block_Address_Type (Primitive.Block_Number (Obj.Curr_Query_Prim));
                        end if;
                        Declare_Free_Blocks :
                        declare
            ...
            --  break off early
            exit For_Type_2_Nodes when Obj.Nr_Of_Blocks = Obj.Nr_Of_Found_Blocks;
      ...
      declare
         End_Of_Tree : constant Boolean :=
            Primitive.Block_Number (Obj.Curr_Query_Prim) + Block_Number_Type (Tree_Helper.Degree (Obj.Trans_Helper))
                  ...
                  for Tree_Level_1 in 1 .. Tree_Helper.Height (Obj.Trans_Helper) loop
                     ...
                     begin
                        if Type_2_Node then
                           ...
                           declare
                              Nodes : Type_Ii_Node_Block_Type with
                                 Address => Cach_Data (Data_Index)'Address;
                           begin
                              For_Nodes_1 :
                              for Node_Index in 0 .. Tree_Helper.Degree (Obj.Trans_Helper) - 1 loop
                                 ...
                                 for Tree_Level_2 in 0 .. Tree_Helper.Height (Obj.Trans_Helper) loop
                                    if Nodes (Natural (Node_Index)).Pba
                                       = Obj.Wb_Data.New_Pbas (Tree_Level_Index_Type (Tree_Level_2))
                                    then
                                       Nodes (Natural (Node_Index)).Pba :=
                                          Obj.Wb_Data.Old_Pbas (Natural (Tree_Level_2)).Pba;
                                       Nodes (Natural (Node_Index)).Alloc_Gen :=
                                          Obj.Wb_Data.Old_Pbas (Natural (Tree_Level_2)).Gen;
                           ...
                           declare
                              ...
                              Pre_Hash_Data : Sha256_4k.Data_Type with
                                 Address => Cach_Data (Pre_Data_Index)'Address;
                              Nodes : Type_I_Node_Block_Type with
                                 Address => Cach_Data (Data_Index)'Address;
                           begin
                              ...
                              if Tree_Level_1 = Tree_Helper.Height (Obj.Trans_Helper) then
                                 Declare_Hash_Data :
                                 declare
                                    Hash_Data : Sha256_4k.Data_Type with
                                       Address => Cach_Data (Data_Index)'Address;
                                 begin
   ...
   procedure Execute_Writeback (Obj : in out Object_Type) is
   begin
      Declare_Wb_Ongoing :
      declare
      ...
      begin
         ...
         for Wb_Io_Index in Tree_Level_Index_Type'Range loop
            Wb_Ongoing :=
               Wb_Ongoing
               or
               (Obj.Wb_Ios (Wb_Io_Entries_Index_Type (Wb_Io_Index)).State = Pending
                or Obj.Wb_Ios (Wb_Io_Entries_Index_Type (Wb_Io_Index)).State = In_Progress);
   ...
   function Peek_Generated_Primitive (Obj : Object_Type) return Primitive.Object_Type
   is
   begin
      --  current type 2 node
   ...
   function Peek_Generated_Data_Index (Obj  : Object_Type;
   ...
   function Peek_Completed_Wb_Data (Obj  : Object_Type;
                                    Prim : Primitive.Object_Type) return Write_Back_Data_Type
   is
   begin
