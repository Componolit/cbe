...
package body Cbe.Virtual_Block_Device with
   Spark_Mode
is
   ...
   --  Initialized_Object
   --
   function Initialized_Object (Height : Tree_Level_Type;
                                Degree : Tree_Degree_Type;
                                Leafs  : Tree_Number_Of_Leafs_Type) return Object_Type
   is
      Trans_Helpr : constant Tree_Helper.Object_Type := Tree_Helper.Initialized_Object (Degree, Height, Leafs);
   begin
      return (Trans_Helper => Trans_Helpr, Trans => Translation.Initialized_Object (Trans_Helpr, False),
   ...
   --
   procedure Trans_Inhibit_Translation (Obj : in out Object_Type) is
   begin
   ...
   --
   procedure Trans_Resume_Translation (Obj : in out Object_Type) is
   begin
   ...
   --
   function Trans_Get_Virtual_Block_Address (Obj  : Object_Type;
                                             Prim : Primitive.Object_Type) return Block_Number_Type
   is
   begin
      if Primitive.Block_Number (Prim) /= Block_Number_Type (Translation.Data_Pba (Obj.Trans)) then
         raise Program_Error;
      end if;
   ...
   --
   function Trans_Can_Get_Type_1_Info (Obj   : Object_Type;
                                       Prim  : Primitive.Object_Type;
                                       Infos : Address_Type) return Boolean is
      (Translation.Can_Get_Type_1_Info (Obj.Trans, Prim, Infos));

   --
   --  Trans_Can_Get_Type_1_Info_Spark
   --
   function Trans_Can_Get_Type_1_Info_Spark (Obj  : Object_Type;
                                             Prim : Primitive.Object_Type) return Boolean is
      (Translation.Can_Get_Type_1_Info_Spark (Obj.Trans, Prim));

   --
   --  Trans_Get_Type_1_Info
   --
   procedure Trans_Get_Type_1_Info (Obj   :        Object_Type;
   ...
   --  Tree_Height
   --
   function Tree_Height (Obj : Object_Type) return Tree_Level_Type is
      (Tree_Helper.Height (Obj.Trans_Helper));

   --
   --  Index_For_Level
   --
   function Index_For_Level (Obj   : Object_Type;
                             Vba   : Virtual_Block_Address_Type;
                             Level : Tree_Level_Type) return Tree_Child_Index_Type is
      (Tree_Helper.Index (Obj.Trans_Helper, Vba, Level));

   --
   --  Get_Tree_Helper
   --
   function Get_Tree_Helper (Obj : Object_Type) return Tree_Helper.Object_Type is (Obj.Trans_Helper);

   --
   --  Primitive_Acceptable
   --
   function Primitive_Acceptable (Obj : Object_Type) return Boolean is (Translation.Acceptable (Obj.Trans));
   ...
   begin
      ...
      declare
         Trans_Progress : constant Boolean := Translation.Execute_Progress (Obj.Trans);
      begin
         Obj.Execute_Progress := Obj.Execute_Progress or Trans_Progress;
            ...
            begin
               if not Cache.Data_Available (Cach, Pba) then
                  ...
                  declare
                     ...
                     Data       : constant Block_Data_Type        := Cach_Data (Data_Index);
                  begin
                     Translation.Mark_Generated_Primitive_Complete (Obj.Trans, Data, Trans_Data);
                  end Declare_Data;
   ...
   is
   begin
      return "VBD(Execute_Progress=" & Cbe.To_String (Obj.Execute_Progress) & ", Trans="
