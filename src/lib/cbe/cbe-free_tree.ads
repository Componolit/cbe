...
is
   ...
   function Peek_Generated_Data_Index (Obj  : Object_Type;
                                       Prim : Primitive.Object_Type) return Index_Type;
...
private
   ...
   function Query_Branch_Invalid return Query_Branch_Type is
      (Trans_Infos => (others => Type_1_Node_Info_Invalid), Pbas => (others => Pba_Invalid), Nr_Of_Free_Blocks => 0,
       Vba         => Vba_Invalid);
   function Io_Entry_Invalid return Io_Entry_Type is (Pba => Pba_Invalid, State => Invalid, Index => 0);
   function Write_Back_Data_Invalid return Write_Back_Data_Type is
      (Prim     => Primitive.Invalid_Object, Gen => 0, Vba => Vba_Invalid, Tree_Height => 0,
       New_Pbas => (others => Pba_Invalid), Old_Pbas => (others => Type_1_Node_Info_Invalid), Finished => False);
   ...
   procedure Execute_Update (Obj       : in out Object_Type;
                             ...
                             Cach_Data : in out Cache.Cache_Data_Type;
                             Timestamp :        Timestamp_Type);
   procedure Execute_Writeback (Obj : in out Object_Type);
