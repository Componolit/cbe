...
is
   ...
   is
      ...
      --  Primitive_Completed
      --
      procedure Primitive_Completed (Obj  : in out Item_Type;
      ...
      function Invalid_Object return Item_Type is
         (State => Invalid, Req => Request.Invalid_Object, Nr_Of_Prims => 0, Nr_Of_Done_Prims => 0);

      --
      --  Pending_Object
      --
      function Pending_Object (Rq               : Request.Object_Type;
                               Nr_Of_Prims      : Number_Of_Primitives_Type;
                               Nr_Of_Done_Prims : Number_Of_Primitives_Type) return Item_Type is
         (State => Pending, Req => Rq, Nr_Of_Prims => Nr_Of_Prims, Nr_Of_Done_Prims => Nr_Of_Done_Prims);
      ...
      procedure Req (Obj : in out Item_Type;
                     Rq  :        Request.Object_Type)
      is
   ...
   --  Initialized_Object
   --
   function Initialized_Object return Object_Type is (Items => (others => Item.Invalid_Object));
   ...
   begin
      ...
      for Item_Id in Obj.Items'Range loop
         if Item.Invalid (Obj.Items (Item_Id)) then
            ...
            Obj.Items (Item_Id) :=
               Item.Pending_Object (Rq => Req_Buf, Nr_Of_Prims => Nr_Of_Prims, Nr_Of_Done_Prims => 0);
            exit Items_Loop;
         end if;
      end loop Items_Loop;
   end Submit_Request;
   ...
   --
   function Peek_Pending_Request (Obj : Pool.Object_Type) return Request.Object_Type
   is
   begin
      for Item_Id in Obj.Items'Range loop
   ...
   --
   function Peek_Completed_Request (Obj : Pool.Object_Type) return Request.Object_Type
   is
   begin
      for Item_Id in Obj.Items'Range loop
   ...
   --  Request_For_Tag
   --
   function Request_For_Tag (Obj : Object_Type;
                             Tag : Tag_Type) return Request.Object_Type
   is
      Req : Request.Object_Type;
   begin
      for Itm of Obj.Items loop
