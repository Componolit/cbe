...
is
   ...
   procedure Print_U8 (U : U8) with
      Import,
      Convention    => C,
      External_Name => "print_u8";

   procedure Print_U16 (U : U16) with
      Import,
      Convention    => C,
      External_Name => "print_u16";

   procedure Print_U32 (U : U32) with
      Import,
      Convention    => C,
      External_Name => "print_u32";

   procedure Print_U64 (U : U64) with
      Import,
      Convention    => C,
      External_Name => "print_u64";

   procedure Print_Cstring (S      : String;
                            Length : U64) with
      Import,
      Convention    => C,
   ...
   type Byte_Type is range 0 .. 255 with
      Size => 8;

   type Block_Data_Type is array (0 .. 4095) of Byte_Type with
      Size => 4096 * 8;

   type Translation_Data_Type is array (0 .. 0) of Block_Data_Type with
      Size => 1 * 4096 * 8;
   ...
   type Hash_Type is array (1 .. 32) of Byte_Type with
      Size => 32 * 8;
   ...
   type Type_I_Node_Block_Type is array (0 .. (Block_Data_Type'Size / Type_I_Node_Type'Size) - 1) of Type_I_Node_Type;
   ...
   type Type_Ii_Node_Block_Type is
      array (0 .. (Block_Data_Type'Size / Type_Ii_Node_Type'Size) - 1) of Type_Ii_Node_Type;
   pragma Pack (Type_Ii_Node_Block_Type);

   type Query_Data_Type is array (0 .. 0) of Block_Data_Type with
      Size => 1 * 4096 * 8;
      --
   ...
   for Snapshot_Type'Size use 32 * 8 + --  Hash
   8 * 8 + --  PBA
   8 * 8 + --  Gen
   8 * 8 + --  Nr_Of_Leafs
   4 * 8 + --  Height
   ...
   function Tree_Min_Degree return Tree_Degree_Type is (1);
   function Tree_Min_Height return Tree_Level_Type is (1);
   function Tree_Max_Height return Tree_Level_Type is (Tree_Level_Type (Tree_Level_Index_Type'Last) + 1);
   function Snapshot_Id_Invalid return Snapshot_Id_Type is (Snapshot_Id_Type'Last);
   function Snapshot_Flags_Keep_Mask return Snapshot_Flags_Type is (1);
   function Snapshot_Valid (Snap : Snapshot_Type) return Boolean is
      (Snap.Id /= Snapshot_Id_Invalid);
   function Snapshot_Keep (Snap : Snapshot_Type) return Boolean is
      ((Snap.Flags and Snapshot_Flags_Keep_Mask) /= 0);
   procedure Snapshot_Discard (Snap : in out Snapshot_Type);

   type Snapshots_Index_Type is range 0 .. 47;

   type Snapshots_Type is array (Snapshots_Index_Type) of Snapshot_Type with
      Size => 48 * 72 * 8;

   type Type_1_Node_Infos_Type is array (0 .. Natural (Tree_Level_Index_Type'Last)) of Type_1_Node_Info_Type;
   function Snapshot_Id_Invalid_Slot return Snapshot_Id_Type is (Snapshot_Id_Type (Snapshots_Index_Type'Last) + 1);
   function Type_1_Node_Info_Invalid return Type_1_Node_Info_Type is (Pba => 0, Gen => 0, Hash => (others => 0));
   ...
   type Key_Value_Type is array (Key_Value_Index_Type) of Byte_Type with
      Size => 64 * 8;
      --
   ...
   type Super_Blocks_Type is array (Super_Blocks_Index_Type) of Super_Block_Type with
      Size => 8 * 4096 * 8;
